...
FUSEKI_DATASET="<dataset-name>"                # e.g. from datasets.md
```
...
docker run -d -p $GRAPH_EXPLORER_PORT:80 \
  ...
  --env PROXY_SERVER_HTTPS_CONNECTION=false \
  --env GRAPH_EXP_HTTPS_CONNECTION=false \
  --env PUBLIC_OR_PROXY_ENDPOINT=$FUSEKI_URL/$FUSEKI_DATASET \
...
- **SPARQL endpoint path.** Use the dataset path in the URL: `$FUSEKI_URL/$FUSEKI_DATASET` not `$FUSEKI_URL/$FUSEKI_DATASET/sparql`. Graph Explorer appends the appropriate endpoint suffix automatically.
