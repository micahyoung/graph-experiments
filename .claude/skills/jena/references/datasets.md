...
For a local Docker instance started via `references/local-docker.md`, `$FUSEKI_URL` is the host/port from that setup and `$FUSEKI_CREDENTIALS` is `admin:<extracted-password>`. For any other Fuseki, substitute whatever the user supplies.
...
- **Security configuration** — the image used in `references/local-docker.md` ships with sensible defaults; production hardening is out of scope.
